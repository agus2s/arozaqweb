<!DOCTYPE html>
<html lang="id">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Bani K.H. Abdurrozaq</title>
    <link rel="stylesheet" href="styles.css">
</head>
<body>
    <?php include 'header.php'; ?>
    <main>
        <section>
            <h2>Ibu Hamidah bersuami Bapak Abu Nangim</h2>
            <p>Ibu Hamidah anak dari Ibu Mariyah dan Bapak Abdullah Umar. Ibu Hamidah bersuami Bapak Abu Nangim. Beliau memiliki 9 anak:</p>
            <ol>
                <li><a href="#">Bapak Mujahid</a></li>
                <li><a href="#">Ibu Mutingah</a></li>
                <li><a href="#">Bapak Muhtarom</a></li>
                <li><a href="#">Bapak Mungawam</a></li>
                <li><a href="#">Bapak Muhtadi</a></li>
                <li><a href="#">Bapak Fatoni</a></li>
                <li><a href="#">Ibu Mutasingah</a></li>
                <li><a href="#">Bapak Muhtasun</a></li>
                <li><a href="#">Ibu Khoiriyah</a></li>
            </ol>
        </section>
        <section>
            <h2>Garis Keturunan</h2>
            <div class="tree">
                <div class="node"><a href="../index.php">Bapak K.H. Abdurrozaq</a></div>
                <div class="line"></div>
                <div class="node"><a href="index.php">Ibu Mariyah</a></div>
                <div class="line"></div>
                <div class="node">Ibu Hamidah</div>
            </div>
        </section>
    </main>
    <?php include '../footer.php'; ?>
</body>
</html>